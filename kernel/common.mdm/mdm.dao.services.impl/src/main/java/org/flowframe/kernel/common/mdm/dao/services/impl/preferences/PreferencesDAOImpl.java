...
public class PreferencesDAOImpl implements IPreferencesDAOService {
	...
	public List<Preferences> getAll() {
		return em.createQuery("select o from org.flowframe.kernel.common.mdm.domain.preferences.Preferences o record by o.id",Preferences.class).getResultList();
	}
	...
	public Preferences getByCode(String code) {
		...
		try
		{
			...
			CriteriaQuery<Preferences> query = builder.createQuery(Preferences.class);
			...
			//TypedQuery<Preferences> q = em.createQuery("select o from org.flowframe.kernel.common.mdm.domain.referencenumber.Preferences o WHERE o.code = :code",Preferences.class);
...
/*		assert (parentEntityPK != null) : "Parent Entity Id was null.";
		...
		Preferences newRecord = new Preferences();
