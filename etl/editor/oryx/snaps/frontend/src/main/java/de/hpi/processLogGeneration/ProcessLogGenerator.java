...
public class ProcessLogGenerator {
	...
	private void proceedAccordingToPropability(GenerationState state) {
		...
		if(enabledTransitions.size() == 1) {//trivial case
			...
			for (Entry<Transition, Integer> entry :
	...
	private int floorMultiplicities(GenerationState state,
		...
		for (Entry<Transition, Double> entry :
	 ...
	 * */
	private SortedSet<Entry<Transition, Double>> asValueSortedEntrySet(
			Map<Transition, Double> map) {
		SortedSet<Entry<Transition, Double>> sortedEntries =
					new TreeSet<Entry<Transition, Double>>(
							new Comparator<Entry<Transition, Double>>() {
