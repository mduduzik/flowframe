		...
		for (IControlFlow edge : (Collection<IControlFlow>)diag.getControlFlow()) {
			if(Marking.isStartArc(edge, diag)){
				if(startNodes.contains(edge.getSource())){
					...
					marking.applyContext(edge, Context.WAIT);
					marking.applyState(edge, State.POS_TOKEN);
				} else {
					// If not enabled start arc
					marking.applyContext(edge, Context.DEAD);
					marking.applyState(edge, State.NEG_TOKEN);
				}
			} else {
				// If normal arc
				marking.applyContext(edge, Context.WAIT);
				marking.applyState(edge, State.NO_TOKEN);
			}
		}
