...
-->
  ...
  <xsl:param name="doc">/xslt/config.xml</xsl:param>
  <xsl:param name="confDoc"></xsl:param>
  <xsl:param name="config" select="document($confDoc)"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="initial_JSON">
      <xsl:apply-templates select="self::node()/child::*" mode="build" >
        ...
        <xsl:for-each select="../*[name()=$nName]">
          ...
          <xsl:variable name="nonSimpleChildrenCount">
            <xsl:call-template name="support-count-nonsimple-children">
              <xsl:with-param name="startNode" select="self::node()"/>
            </xsl:call-template>
          </xsl:variable>
          ...
          <xsl:variable name="json_content">
            <xsl:apply-templates select="self::node()" mode="process-node">
          ...
          <xsl:if test="child::*">
            ...
            <xsl:apply-templates select="self::node()/*" mode="build">
  ...
  <xsl:template name="support-count-nonsimple-children">
    ...
    <xsl:variable name="total">
      ...
      <xsl:for-each select="$startNode">
        ...
        <xsl:value-of select="number(count(self::node()/*)) +
                              number(count(self::node()[@*])) +
