...
@Step(
        id = "RepoFileToResult",
        image = "ui/images/FTR.png",
        description = "RepoFileToResult.Description",
        name = "RepoFileToResult.Name",
        categoryDescription = "RepoFileToResult.Category",
        i18nPackageName = "org.flowframe.etl.pentaho.plugin.di.trans.steps.docrepo.copyrepofile"
)

public class RepoFileToResultMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepinfo, RowMetaInterface prev, String input[], String output[], RowMetaInterface info)
	{
		...
		if (input.length>0)
		{
			CheckResult cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "RepoFileToResultMeta.CheckResult.StepReceivingInfoFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
		else
		{
			CheckResult cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "RepoFileToResultMeta.CheckResult.NoInputReceivedError"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	...
	{
		return new RepoFileToResult(stepMeta, stepDataInterface, cnr, tr, trans);
	}

	public StepDataInterface getStepData()
	{
		return new RepoFileToResultData();
	}
