package org.flowframe.ui.pageflow.vaadin.builder;
...
import org.flowframe.ui.pageflow.vaadin.ext.form.VaadinConfirmActualsForm;
import org.flowframe.ui.pageflow.vaadin.ext.grid.VaadinMatchGrid;
import org.flowframe.ui.pageflow.vaadin.ext.grid.VaadinMatchGrid.IMatchListener;
import org.flowframe.ui.pageflow.vaadin.ext.mvp.IVaadinDataComponent;
import org.flowframe.ui.pageflow.vaadin.mvp.editor.MasterSectionEventBus;
...
import org.flowframe.ui.pageflow.vaadin.mvp.lineeditor.EntityLineEditorEventBus;
import org.flowframe.ui.pageflow.vaadin.mvp.lineeditor.view.EntityLineEditorView;
...
//			}
	...
	*/
	...
	public void applyData(Map<String, Object> config, Component componentRoot) {
		if (componentRoot instanceof EntityGrid) {
			DataSource ds = ((EntityGrid) componentRoot).getComponentModel().getDataSource();
			// TODO Replace the String Literal
			IMainApplication mainApp = (IMainApplication) config.get(IComponentFactory.FACTORY_PARAM_MAIN_APP);
			IEntityContainerProvider provider = mainApp.getContainerProvider();
			try {
				EntityContainer<?> container = (EntityContainer<?>) provider.createPersistenceContainer(ds.getEntityType().getJavaType());
				User currentUser = mainApp.getCurrentUser();
				Filter filter = SPELUtil.toContainerFilter(currentUser, mainApp.findDAOByClass(IPortalRoleService.class), ds.getDefaultFilterExpression(), new HashMap<String, Object>());
				container.addContainerFilter(filter);
				((EntityGrid) componentRoot).setContainerDataSource(container);
			} catch (ClassNotFoundException e) {
				throw new RuntimeException(e);
			}
		}  else {
			// Component containers should always be picked last
			if (componentRoot instanceof FlowFrameVerticalSplitPanel) {
				applyData(config, ((FlowFrameVerticalSplitPanel) componentRoot).getFirstComponent());
				applyData(config, ((FlowFrameVerticalSplitPanel) componentRoot).getSecondComponent());
			}			
			else if (componentRoot instanceof TabSheet) {
				Iterator<Component> it = ((TabSheet) componentRoot).getComponentIterator();
				Component child;
				while (it.hasNext()) {
					child = it.next();
					applyData(config, child);
				}
			}
			 else if (componentRoot instanceof AbstractComponentContainer) {
				Iterator<Component> it = ((AbstractComponentContainer) componentRoot).getComponentIterator();
				Component child;
				while (it.hasNext()) {
					child = it.next();
					applyData(config, child);
