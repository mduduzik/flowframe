package org.flowframe.ui.vaadin.ext.feature;
...
public abstract class MultiFeatureView extends TabSheet implements IFeatureView {
	...
	public void openFeature(Feature feature) {
		if (!isFeatureOpen(feature)) {
			...
			createAndAddFeatueTab(feature);
		}
		else
		{
			Tab featureTab = this.featureMap.get(feature);
			if (this.getTab(featureTab.getComponent()) == null)
			{
	...
	private void createAndAddFeatueTab(Feature feature) {
		...
		Tab featureTab = null;
		if (iconUrl != null) {
			featureTab = addTab(featureComponent, name, new ThemeResource(iconUrl));
		} else {
			featureTab = addTab(featureComponent, name);
		}
		assert (featureTab != null) : "Could not add a tab for this feature.";

		featureTab.setClosable(true);
