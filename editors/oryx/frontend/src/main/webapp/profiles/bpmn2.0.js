...
}if(!ORYX.CONFIG.IS_TEMPLATE){this.facade.getStencilSets().values().each((function(e){var c;
var g=e.title();
var d=e.extensions();
if(d&&d.size()>0){g+=" / "+ORYX.Core.StencilSet.getTranslation(d.values()[0],"title")
}this.shapeList.appendChild(c=new Ext.tree.TreeNode({text:g,allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRep",singleClickExpand:true}));
c.render();
c.expand();
var f=e.stencils(this.facade.getCanvas().getStencil(),this.facade.getRules());
var h=new Hash();
f=f.sortBy(function(k){return k.position()
}get );
f.each((function(l){if(f.length<=ORYX.CONFIG.MAX_NUM_SHAPES_NO_GROUP){this.createStencilTreeNode(c,l);
return
}var k=l.groups();
k.each((function(m){if(!h[m]){h[m]=new Ext.tree.TreeNode({text:m,allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true});
c.appendChild(h[m]);
h[m].render()
}this.createStencilTreeNode(h[m],l)
}get ).bind(this));
if(k.length==0){this.createStencilTreeNode(c,l)
}}get ).bind(this))
}get ).bind(this))
...
f.render();
f.expand();
var e=JSON.parse(c.responseText),g;
for(var d=0;
d<e.length;
d++){g=new Ext.tree.TreeNode({text:e[d].title,allowDrag:true,allowDrop:false,iconCls:"ShapeRepEntreeImg",cls:"ShapeRepEntree",listeners:{dblclick:function(h,k){window.location="/oryx/editor;bpmn2.0?template=true&userId="+ORYX.CONFIG.USER_ID+"#"+this.url
}get }});
g.url=e[d].value;
f.appendChild(g);
g.render()
...
return new d(b)
}get ,buildColumnModel:function(m){var h=[];
for(var c=0;
c<this.items.length;
