package org.flowframe.ui.vaadin.common.editors.data;
...
import org.flowframe.ui.vaadin.common.editors.ext.form.VaadinCollapsibleConfirmActualsForm;
import org.flowframe.ui.vaadin.common.editors.ext.form.VaadinConfirmActualsForm;
import org.flowframe.ui.vaadin.common.editors.ext.grid.VaadinMatchGrid;
import org.flowframe.ui.vaadin.common.editors.ext.grid.VaadinMatchGrid.IMatchListener;
import org.flowframe.ui.vaadin.common.editors.ext.mvp.IVaadinDataComponent;
import org.flowframe.ui.vaadin.common.editors.mvp.editor.MasterSectionEventBus;
...
import org.flowframe.ui.vaadin.common.editors.mvp.lineeditor.EntityLineEditorEventBus;
import org.flowframe.ui.vaadin.common.editors.mvp.lineeditor.view.EntityLineEditorView;
...
public class VaadinEditorDataManager implements IEditorDataManager {
	private Logger logger = LoggerFactory.getLogger(VaadinEditorDataManager.class);
	...
	public Set<Object> buildResultData(Component component) {
		...
		if (component instanceof IVaadinDataComponent) {
			...
			{
				logger.warn("Component "+component.getClass().getName()+"not covered by VaadinEditorDataManager.buildResultData");
			}
		}
	...
	public void applyParamData(Map<String, Object> config, Component component, Map<String, Object> params,
			Object presenterFactory_) throws Exception {
		PresenterFactory presenterFactory = (PresenterFactory)presenterFactory_;
	...
	public void applyItemDataSource(boolean isEventDeclaritive, Component component, Container itemContainer, Item item,
		...
		} else if (component instanceof VaadinEntityPreferenceForm)
		{
			...
			IDAOProvider daoProvider = (IDAOProvider) config.get(IComponentFactory.CONTAINER_PROVIDER);
			...
			if (item instanceof BeanItem && itemContainer instanceof BeanItemContainer) {
				...
				IEntityContainerProvider containerProvider = (IEntityContainerProvider) config
						.get(IComponentFactory.CONTAINER_PROVIDER);
				...
				if (config.get(IComponentFactory.FACTORY_PARAM_MVP_EVENTBUS_MANAGER) != null) {
					ebm = (EventBusManager) config.get(IComponentFactory.FACTORY_PARAM_MVP_EVENTBUS_MANAGER);
				} else {
			...
			IEntityContainerProvider provider = (IEntityContainerProvider) config.get(IComponentFactory.CONTAINER_PROVIDER);
			IDAOProvider daoProvider = (IDAOProvider) config.get(IComponentFactory.CONTAINER_PROVIDER);
...
//			}
	...
	private void applyItemDataSourceToVaadinForm(boolean isEventDeclaritive, VaadinForm form, BeanItem<?> item,
		...
		IDAOProvider daoProvider = (IDAOProvider) config.get(IComponentFactory.CONTAINER_PROVIDER);
		IEntityContainerProvider containerProvider = (IEntityContainerProvider) config.get(IComponentFactory.CONTAINER_PROVIDER);
