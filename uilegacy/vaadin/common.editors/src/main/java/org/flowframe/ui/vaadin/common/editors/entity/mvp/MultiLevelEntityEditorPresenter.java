...
public class MultiLevelEntityEditorPresenter extends ConfigurableBasePresenter<IMultiLevelEntityEditorView, MultiLevelEntityEditorEventBus> implements
		ISplitPositionChangeListener {
	...
	public void onReportItem(Object itemEntity) {
		...
		// if (app instanceof MainMVPApplication && itemEntity instanceof
		// StockItem) {
		// String url = ((MainMVPApplication)
		// app).getDaoProvider().provideByDAOClass(IStockItemDAOService.class)
		// .getStockItemLabelUrl((StockItem) itemEntity, ((MainMVPApplication)
		// app).getReportingUrl());
		// if (url != null) {
		// this.appEventBus.openDocument(url, "Label for " + ((StockItem)
		// itemEntity).getCode());
		// }
		// }
	}
	...
	public void onEditItem(Item item, MasterDetailComponent componentModel) {
		...
		if (childEditorPresenter == null) {
			...
			Map<IPresenter<?, ? extends EventBus>, EventBus> res = (Map<IPresenter<?, ? extends EventBus>, EventBus>) this.entityFactory.create(componentModel,
					getConfig());
...
//		if (!this.isConfigured) {
			...
			Map<IPresenter<?, ? extends EventBus>, EventBus> res = (Map<IPresenter<?, ? extends EventBus>, EventBus>) this.entityFactory.create(
					this.metaData.getMasterComponent(), getConfig());
...
//		}
	...
	protected IComponentModelFactory createComponentFactory() {
		this.entityFactory = this.componentFactoryManager.create(new HashMap<String, Object>(), presenterFactory);// new
																													// VaadinEntityEditorFactoryImpl(presenterFactory);
